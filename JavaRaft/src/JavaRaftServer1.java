...
public class JavaRaftServer1 {

	public static void main(String[] args) {
		List<Address> members = Arrays.asList(
				new Address("127.0.0.1", 5555)
				);
		RaftServer server = RaftServer.builder(new Address("127.0.0.1", 6666), members)
				.withTransport(new NettyTransport())
				.withStorage(new Storage("logs"))
				.withStateMachine(new MyStateMachine())
				.build();
		server.open().thenRun(() -> System.out.println("Server 1 started successfully!"));
	}
